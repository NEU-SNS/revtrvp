 ...
 * scamper_task.c
 *
 * $Id: scamper_task.c,v 1.59 2015/04/27 15:20:22 mjl Exp $
 *
...
{
  if((tx_ip = splaytree_alloc((splaytree_cmp_t)tx_ip_cmp)) == NULL)
    return -1;
  if((tx_nd = splaytree_alloc((splaytree_cmp_t)tx_nd_cmp)) == NULL)
    return -1;
