 ...
 * scamper_tbit.h
 *
 * $Id: scamper_tbit.h,v 1.52 2015/08/29 03:58:58 mjl Exp $
 *
...
/* blind options */
#define SCAMPER_TBIT_BLIND_OPTION_SPLIT      0x01 /* split the blind data */
...
typedef struct scamper_tbit_abc
{
  uint8_t              icw;
  uint8_t              limit;
  uint32_t             last_ack;
...
scamper_tbit_abc_t *scamper_tbit_abc_alloc(uint8_t ackc);
void scamper_tbit_abc_free(scamper_tbit_abc_t *abc);
int scamper_tbit_abc_size(const scamper_tbit_t *tbit, uint32_t *size);
int scamper_tbit_abc_setacks(scamper_tbit_abc_t *abc, const uint8_t *acks,
			     uint8_t ackc);
