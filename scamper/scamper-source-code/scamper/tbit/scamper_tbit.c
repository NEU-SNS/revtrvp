 ...
 *
 * $Id: scamper_tbit.c,v 1.45 2015/04/24 04:34:19 mjl Exp $
 *
...
int scamper_tbit_abc_setacks(scamper_tbit_abc_t *abc,
			     const uint8_t *acks, uint8_t ackc)
{
  if((abc->acks = memdup(acks, ackc)) == NULL)
    return -1;
  abc->ackc = ackc;
  return 0;
}

int scamper_tbit_abc_size(const scamper_tbit_t *tbit, uint32_t *abc_out)
{
  const scamper_tbit_abc_t *abc = tbit->data;
  const scamper_tbit_pkt_t *pkt;
  scamper_tbit_tcpq_t *q = NULL;
  uint32_t i, seq, u32, first_seq;
  uint16_t iplen, datalen;
  uint8_t proto, iphlen, tcphlen, flags, first_seq_c = 0;
  int rc = -1;

  if(tbit->result != SCAMPER_TBIT_RESULT_ABC_SUCCESS ||
     tbit->pktc < 1 || abc->last_ack == 0)
    goto done;
  ...
  first_seq = bytes_ntohl(pkt->data+iphlen+4) + abc->last_ack;
  if((q = scamper_tbit_tcpq_alloc(first_seq)) == NULL)
    goto done;
  ...
  for(i++; i<tbit->pktc; i++)
	...
	goto done;
      ...
      /* skip over rx packets with no data */
      ...
      if(first_seq_c == 0)
	{
      ...
      else if(first_seq_c == 1 && seq == first_seq)
	{
	  u32 = scamper_tbit_tcpq_tail(q);
	  *abc_out = scamper_tbit_data_seqoff(first_seq, u32);
	  rc = 0;
	  break;
	}

      if(scamper_tbit_data_inrange(first_seq, seq, datalen) == 0)
	continue;
  ...
  if(q != NULL) scamper_tbit_tcpq_free(q, NULL);
  return rc;
}
...
scamper_tbit_abc_t *scamper_tbit_abc_alloc(uint8_t ackc_in)
{
  scamper_tbit_abc_t *abc;
  if((abc = malloc_zero(sizeof(scamper_tbit_abc_t))) == NULL)
    return NULL;
  if(ackc_in > 0 &&
     (abc->acks = malloc_zero(sizeof(uint8_t) * ackc_in)) == NULL)
    {
      free(abc);
      return NULL;
    }
...
void scamper_tbit_abc_free(scamper_tbit_abc_t *abc)
{
  if(abc == NULL) return;
  if(abc->acks != NULL) free(abc->acks);
  free(abc);
  return;
}
...
void scamper_tbit_free(scamper_tbit_t *tbit)
{
  ...
  if(tbit->data != NULL)
	...
	scamper_tbit_abc_free(tbit->data);
