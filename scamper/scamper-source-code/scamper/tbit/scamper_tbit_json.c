 ...
 *
 * $Id: scamper_tbit_json.c,v 1.19 2015/08/28 05:14:00 mjl Exp $
 *
			       ...
			       const tbit_state_t *state)
{
  ...
  if(tbit->type == SCAMPER_TBIT_TYPE_PMTUD)
  ...
  else if(tbit->type == SCAMPER_TBIT_TYPE_ABC)
    {
      abc = tbit->data;
      string_concat(buf, sizeof(buf), &off,
		    ", \"icw_segments\":%u, \"limit\":%u",
		    abc->icw, abc->limit);
      if(abc->last_ack != 0)
	string_concat(buf,sizeof(buf),&off,", \"last_ack\":%u",abc->last_ack);
      if(tbit->result == SCAMPER_TBIT_RESULT_ABC_SUCCESS &&
	 scamper_tbit_abc_size(tbit, &u32) == 0)
	string_concat(buf, sizeof(buf), &off, ", \"abc_bytes\":%u", u32);
    }
