 ...
 * scamper_do_tbit.c
 *
 * $Id: scamper_tbit_do.c,v 1.164 2015/08/29 03:58:58 mjl Exp $
 *
...
#define abc_seq               un.abc.seq
#define abc_count             un.abc.count
#define abc_mode              un.abc.mode
#define abc_tail              un.abc.tail
#define abc_ackx              un.abc.ackx
...
#define blind_f_half          un.blind.f_half
#define blind_s_half          un.blind.s_half
...
/* modes for the abc test */
#define TBIT_STATE_ABC_MODE_ICW         0x00
#define TBIT_STATE_ABC_MODE_SECOND      0x01
#define TBIT_STATE_ABC_MODE_FIN         0x02
...
#define TBIT_OPT_ICW         14
#define TBIT_OPT_LIMIT       15
#define TBIT_OPT_ACKS        16
...
#define TBIT_OPT_OPTION_SPLIT      0x0100
...
static const scamper_option_in_t opts[] = {
  {'a', NULL, TBIT_OPT_ACKS,     SCAMPER_OPTION_TYPE_STR},
  ...
  {'i', NULL, TBIT_OPT_ICW,      SCAMPER_OPTION_TYPE_NUM},
  {'L', NULL, TBIT_OPT_LIMIT,    SCAMPER_OPTION_TYPE_NUM},
...
static void tbit_classify(scamper_task_t *task)
{
  ...
  if(tbit->type == SCAMPER_TBIT_TYPE_PMTUD)
	...
	tbit_result(task, SCAMPER_TBIT_RESULT_SACK_RCVR_SUCCESS);
	  ...
	  tbit->type == SCAMPER_TBIT_TYPE_ABC)
    {
      if(state->flags & TBIT_STATE_FLAG_RST_SEEN)
	tbit_result(task, SCAMPER_TBIT_RESULT_TCP_RST);
      else if(state->flags & TBIT_STATE_FLAG_FIN_SEEN)
...
static void dl_syn(scamper_task_t *task, scamper_dl_rec_t *dl)
{
  ...
  else if(tbit->type == SCAMPER_TBIT_TYPE_ABC)
    {
      state->abc_seq = dl->dl_tcp_seq + 1;
    }
  ...
  if(rc > 0)
	...
	{
	  blind = tbit->data;
	  if(blind->options & SCAMPER_TBIT_BLIND_OPTION_SPLIT)
	    {
	      state->blind_f_half = rc / 2;
	      state->blind_s_half = rc - state->blind_f_half;
	      tp->tp_len = state->blind_s_half;
	      tp->tp_seq += state->blind_f_half;
	    }
 ...
 * dl_data_abc
 *
 * walk through the ABC test algorithm.  first, send acks for the ICW.
 * then, do not ack until a retransmission.  finally, ack the whole
 * flight and then change modes to make sure we are not limited by
 * available data.
 */
static int dl_data_abc(scamper_task_t *task, scamper_dl_rec_t *dl)
{
  scamper_tbit_t *tbit = tbit_getdata(task);
  tbit_state_t *state = tbit_getstate(task);
  scamper_tbit_abc_t *abc = tbit->data;
  scamper_tbit_tcpqe_t *qe;
  tbit_probe_t *tp = NULL;
  uint32_t seq;
  uint16_t datalen;
  ...
  if((dl->dl_tcp_flags & TH_FIN) != 0)
    {
      state->flags |= TBIT_STATE_FLAG_FIN_SEEN;
      tbit_result(task, SCAMPER_TBIT_RESULT_ABC_TOOSHORT);
      state->mode = MODE_FIN; fin = 1;
    }
  ...
  if(fin == 0 && state->abc_mode == TBIT_STATE_ABC_MODE_SECOND)
    {
      assert(state->abc_count < 2);
      if(dl->dl_tcp_seq == state->abc_seq && dl->dl_tcp_datalen > 0)
	state->abc_count++;
      if(state->abc_count < 2)
	{
	  if(tbit_rxq(state, dl) != 0)
	    {
	      printerror(errno, strerror, __func__, "could not queue packet");
      ...
      state->abc_tail = scamper_tbit_tcpq_tail(state->rxq);
      state->abc_mode = TBIT_STATE_ABC_MODE_FIN;
      state->mode = MODE_MOREDATA;
      scamper_tbit_tcpq_flush(state->rxq, free);
    }
  ...
  while(scamper_tbit_tcpq_seg(state->rxq, &seq, &datalen) == 0)
      ...
      if(state->flags & TBIT_STATE_FLAG_FIN_SEEN)
	{
      ...
      else if(state->abc_mode == TBIT_STATE_ABC_MODE_ICW)
	{
	  state->abc_count++;
	  if(abc->acks[state->abc_ackx] == state->abc_count)
	    {
	      if(abc->limit == 0 && datalen > 1)
		{
		  datalen = datalen / 2;
		  state->rcv_nxt -= datalen;
		  if((tp = tp_tcp(state)) == NULL)
		    goto err;
		  state->rcv_nxt += datalen;
		}
		...
		goto err;
	      state->abc_ackx++;
	      if(state->abc_ackx == abc->ackc)
		{
		  abc->last_ack = scamper_tbit_data_seqoff(state->abc_seq,
							   state->rcv_nxt) + 1;
		  state->abc_count = 0;
		  state->abc_seq = state->rcv_nxt;
		  state->abc_mode = TBIT_STATE_ABC_MODE_SECOND;
		}
	    }
	}
      else
	{
	  if(state->abc_mode == TBIT_STATE_ABC_MODE_SECOND)
	    {
	      tbit_result(task, SCAMPER_TBIT_RESULT_ABC_BADICW);
	      state->mode = MODE_FIN;
	    }
	  if((tp = tp_tcp(state)) == NULL)
	    goto err;
...
static int dl_data_blinddata(scamper_task_t *task, scamper_dl_rec_t *dl)
{
  ...
  /*
   * if we're not splitting the data request, then note the response
   * as a challenge ack.  we do not consider acks received in a split
   * request as challenge acks because the remote host might be
   * sending an immediate ack for out-of-order data.
   */
  if((blind->options & SCAMPER_TBIT_BLIND_OPTION_SPLIT) == 0)
    {
      state->blind_flags |= TBIT_STATE_BLIND_FLAG_CHALLENGE;
      return 0;
    }

  if((state->flags & TBIT_STATE_BLIND_FLAG_SPLIT) == 0)
    {
      state->flags |= TBIT_STATE_BLIND_FLAG_SPLIT;
      if((tp = tp_tcp(state)) == NULL)
	{
	  tbit_handleerror(task, errno);
	  return -1;
	}
      tp->tp_len = state->blind_f_half;
      tp->wait = TBIT_TIMEOUT_BLINDDATA;
      state->attempt = 0;
      state->blind_c = 1;
    }
...
static void dl_zerowin(scamper_task_t *task, scamper_dl_rec_t *dl)
{
  ...
  if(tbit->type == SCAMPER_TBIT_TYPE_SACK_RCVR)
    tp->tp_len = 1;
	  ...
	  (blind->options & SCAMPER_TBIT_BLIND_OPTION_SPLIT) != 0)
    tp->tp_len = state->blind_s_half;
  else
    tp->tp_len = seg->len;
 ...
 * from beyond the last flight.  dl_data_[icw|abc] has previously sent
      ...
      if(datalen > 0)
	{
	  if(tbit->type == SCAMPER_TBIT_TYPE_ICW &&
	     scamper_tbit_data_seqoff(state->icw_tail, seq+datalen) >= 0)
	    {
		  ...
		  scamper_tbit_data_seqoff(state->abc_tail, seq+datalen) >= 0)
	    {
	      state->mode = MODE_FIN;
	      tbit_result(task, SCAMPER_TBIT_RESULT_ABC_SUCCESS);
	    }
	}
      else
	{
...
static void timeout_blind(scamper_task_t *task)
{
  ...
  if((blind->options & SCAMPER_TBIT_BLIND_OPTION_SPLIT) == 0 ||
     (state->blind_flags & TBIT_STATE_BLIND_FLAG_SPLIT) == 0)
    {
      if((tp = tp_tcp(state)) == NULL)
	goto err;
      tp->tp_len = seg->len;
      ...
      if(state->attempt < blind->retx)
	{
      ...
      else if((blind->options & SCAMPER_TBIT_BLIND_OPTION_SPLIT) == 0)
	{
	  /*
	   * if we did not get a challenge ack during the process of
	   * sending the first packet, then record silence.  a challenge
	   * ack does not make sense as a classification in the split
	   * methodology because an ack received might be an immediate
	   * ack for data it received out of order.
	   */
	...
	{
	  /*
	   * tried all attempts in the split, time to send the first
	   * packet which is missing
	   */
	  state->attempt = 0;
	  state->blind_flags |= TBIT_STATE_BLIND_FLAG_SPLIT;
	  tp->tp_len = state->blind_f_half;
	  state->blind_c = 1;
	}
    }
  else if(state->blind_c == 1)
    {
      if(state->attempt >= blind->retx)
	{
	  tbit_result(task, SCAMPER_TBIT_RESULT_NONE);
      ...
      /* send the missing first half */
      if((tp = tp_tcp(state)) == NULL)
	goto err;
      tp->wait = TBIT_TIMEOUT_BLINDDATA;
      tp->tp_len = state->blind_f_half;
    }
  else if(state->blind_c == 2)
    {
      /* send the missing second half */
      if((tp = tp_tcp(state)) == NULL)
	goto err;
      tp->wait = TBIT_TIMEOUT_BLINDDATA;
      tp->tp_len = seg->len;
...
static int tbit_arg_param_validate(int optid, char *param, long *out)
{
  ...
  switch(optid)
	...
	tmp = SCAMPER_TBIT_TYPE_ICW;
      else if(strcasecmp(param, "abc") == 0)
	tmp = SCAMPER_TBIT_TYPE_ABC;
    ...
    case TBIT_OPT_ICW:
      if(string_tolong(param, &tmp) != 0 || tmp < 1 || tmp > 16)
	goto err;
      break;

    case TBIT_OPT_LIMIT:
      if(string_tolong(param, &tmp) != 0 || tmp < 0 || tmp > 2)
	goto err;
      break;
...
static int tbit_alloc_icw(scamper_tbit_t *tbit, tbit_options_t *o)
{
  ...
  if(o->options & TBIT_OPT_OPTION_TCPTS)
    tbit->options |= SCAMPER_TBIT_NULL_OPTION_TCPTS;
  if(o->options & TBIT_OPT_OPTION_SACK)
    tbit->options |= SCAMPER_TBIT_NULL_OPTION_SACK;
  return 0;
}

static int tbit_alloc_abc(scamper_tbit_t *tbit, tbit_options_t *o)
{
  scamper_tbit_abc_t *abc = NULL;
  char *acks = NULL, *s, *p;
  slist_t *list = NULL;
  long lo, last_lo = 0;
  uint8_t ackc = 0;
  ...
  if(o->icw == 0)
    {
      scamper_debug(__func__, "missing icw option");
      goto done;
    }

  if(o->limit > 2)
    {
      scamper_debug(__func__, "invalid limit option");
      goto done;
    }

  if(o->acks != NULL)
    {
      if((acks = strdup(o->acks)) == NULL)
	{
	  printerror(errno, strerror, __func__, "could not strdup acks");
	  goto done;
	}
      ...
      while(x == 0)
	{
	  if(*p == ',' || *p == '\0')
		...
		x = 1;
	      *p = '\0';
	      if(s == p || string_tolong(s, &lo) != 0 ||
		 lo < 1 || lo > 16 || lo <= last_lo ||
		 slist_tail_push(list, s) == NULL)
		goto done;
	      last_lo = lo;
	      p++; s = p;
	      continue;
  ...
  if((abc = scamper_tbit_abc_alloc(ackc)) == NULL)
    {
      printerror(errno, strerror, __func__, "could not alloc abc struct");
      goto done;
    }
  abc->limit = o->limit;
  abc->icw = o->icw;
  ...
  if(list != NULL)
    {
      while((s = slist_head_pop(list)) != NULL)
	{
	  string_tolong(s, &lo);
	  abc->acks[x] = lo;
	  x++;
    ...
    {
      if(abc->icw == 1 || abc->icw == 3)
	{
	  abc->acks[x++] = 1; abc->acks[x++] = 2; abc->acks[x++] = 3;
	  abc->acks[x++] = 5; abc->acks[x++] = 7;
	}
      else if(abc->icw == 2)
	{
	  abc->acks[x++] = 1; abc->acks[x++] = 2;
	  abc->acks[x++] = 4; abc->acks[x++] = 6;
	}
      else
	{
	  abc->acks[x++] = 2;
  ...
  if(acks != NULL) free(acks);
  if(list != NULL) slist_free(list);
  if(rc != 0) scamper_tbit_abc_free(abc);
  return rc;
}
...
static int tbit_alloc_blind(scamper_tbit_t *tbit, tbit_options_t *o)
{
  ...
  if(tbit->type == SCAMPER_TBIT_TYPE_BLIND_DATA &&
     (o->options & TBIT_OPT_OPTION_SPLIT) != 0)
    blind->options |= SCAMPER_TBIT_BLIND_OPTION_SPLIT;
...
void *scamper_do_tbit_alloc(char *str)
{
  ...
  for(opt = opts_out; opt != NULL; opt = opt->next)
	...
	case TBIT_OPT_OPTION:
	  ...
	  else if(strcasecmp(opt->str, "split") == 0)
	    o.options |= TBIT_OPT_OPTION_SPLIT;
	  else
	    goto err;
	  break;
        }
