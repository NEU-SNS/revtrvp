 ...
 *
 * $Id: scamper_tbit_warts.c,v 1.23 2015/08/28 05:14:00 mjl Exp $
 *
...
#define WARTS_TBIT_ABC_ICW     1
#define WARTS_TBIT_ABC_LIMIT   2
#define WARTS_TBIT_ABC_LASTACK 3
#define WARTS_TBIT_ABC_ACKS    4

static const warts_var_t tbit_abc_vars[] =
{
  {WARTS_TBIT_ABC_ICW,     1, -1},
  {WARTS_TBIT_ABC_LIMIT,   1, -1},
  {WARTS_TBIT_ABC_LASTACK, 4, -1},
  {WARTS_TBIT_ABC_ACKS,   -1, -1},
};
#define tbit_abc_vars_mfb WARTS_VAR_MFB(tbit_abc_vars)
...
typedef struct warts_tbit_abc
{
  uint8_t               flags[WARTS_VAR_MFB(tbit_abc_vars)];
  uint16_t              flags_len;
  uint16_t              params_len;
  uint32_t              len;
} warts_tbit_abc_t;
...
static void warts_tbit_abc_params(const scamper_tbit_t *tbit,
				  warts_tbit_abc_t *state)
{
  ...
  /* unset all the flags possible */
  memset(state->flags, 0, tbit_abc_vars_mfb);
  state->params_len = 0;

  for(i=0; i<sizeof(tbit_abc_vars)/sizeof(warts_var_t); i++)
    {
      var = &tbit_abc_vars[i];
      if((var->id == WARTS_TBIT_ABC_ICW && abc->icw == 0) ||
	 (var->id == WARTS_TBIT_ABC_LIMIT && abc->limit == 0) ||
	 (var->id == WARTS_TBIT_ABC_LASTACK && abc->last_ack == 0) ||
	 (var->id == WARTS_TBIT_ABC_ACKS && abc->ackc == 0))
	continue;

      flag_set(state->flags, var->id, &max_id);

      if(var->id == WARTS_TBIT_ABC_ACKS)
        {
	  state->params_len += (1 + abc->ackc);
	  continue;
        }
...
void insert_abc_acks(uint8_t *buf, uint32_t *off, const uint32_t len,
		     const uint8_t *acks, uint8_t *ackc)
{
  assert(len - *off >= ((*ackc) + 1));
  buf[(*off)++] = *ackc;
  memcpy(buf + *off, acks, *ackc);
  *off += *ackc;
  return;
}

int extract_abc_acks(const uint8_t *buf, uint32_t *off, const uint32_t len,
		     scamper_tbit_abc_t *abc, void *param)
{
  uint8_t ackc;
  if(*off >= len)
    return -1;
  ...
  if(len - ackc < *off)
    return -1;
  if(scamper_tbit_abc_setacks(abc, buf + *off, ackc) != 0)
    return -1;
  *off += ackc;
  return 0;
}
...
static int warts_tbit_abc_read(scamper_tbit_t *tbit, const uint8_t *buf,
			       uint32_t *off, uint32_t len)
{
  scamper_tbit_abc_t *abc = tbit->data;
  warts_param_reader_t handlers[] = {
    {&abc->icw,      (wpr_t)extract_byte,     NULL},
    {&abc->limit,    (wpr_t)extract_byte,     NULL},
    {&abc->last_ack, (wpr_t)extract_uint32,   NULL},
    {abc,            (wpr_t)extract_abc_acks, NULL},
  };
  const int handler_cnt = sizeof(handlers)/sizeof(warts_param_reader_t);
  return warts_params_read(buf, off, len, handlers, handler_cnt);
}

static void warts_tbit_abc_write(const scamper_tbit_t *tbit, uint8_t *buf,
				 uint32_t *off, uint32_t len,
				 warts_tbit_abc_t *state)
{
  scamper_tbit_abc_t *abc = tbit->data;
  warts_param_writer_t handlers[] = {
    {&abc->icw,      (wpw_t)insert_byte,      NULL},
    {&abc->limit,    (wpw_t)insert_byte,      NULL},
    {&abc->last_ack, (wpw_t)insert_uint32,    NULL},
    {abc->acks,      (wpw_t)insert_abc_acks, &abc->ackc}
  };
  const int handler_cnt = sizeof(handlers)/sizeof(warts_param_writer_t);
  warts_params_write(buf, off, len, state->flags, state->flags_len,
		     state->params_len, handlers, handler_cnt);
				 ...
				 scamper_tbit_t **tbit_out)
{
  ...
  switch(tbit->type)
    ...
    case SCAMPER_TBIT_TYPE_ABC:
      if((tbit->data = scamper_tbit_abc_alloc(0)) == NULL)
	goto err;
      break;
  ...
  for(;;)
      ...
      if(junk16 == WARTS_TBIT_STRUCT_TYPE)
	{
	  switch(tbit->type)
	    ...
	    case SCAMPER_TBIT_TYPE_ABC:
	      if(warts_tbit_abc_read(tbit, buf, &i, hdr->len) != 0)
		goto err;
	      break;
				  ...
				  const scamper_tbit_t *tbit)
{
  ...
  if(tbit->data != NULL)
	...
	case SCAMPER_TBIT_TYPE_ABC:
	  warts_tbit_abc_params(tbit, &abc);
	  len += (2 + 4 + abc.len);
	  break;
  ...
  if(tbit->data != NULL)
	...
	case SCAMPER_TBIT_TYPE_ABC:
	  insert_uint32(buf, &off, len, &abc.len, NULL);
	  warts_tbit_abc_write(tbit, buf, &off, len, &abc);
	  break;
