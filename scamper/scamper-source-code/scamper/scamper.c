 ...
 * scamper
 *
 * $Id: scamper.c,v 1.253 2015/08/02 07:57:43 mjl Exp $
 *
 *        Matthew Luckie
 *        mjl@luckie.org.nz
 *
...
static int check_options(int argc, char *argv[])
{
  ...
  off = 0;
  string_concat(opts, sizeof(opts), &off, "c:C:d:e:fF:iIl:L:M:o:O:p:P:R:vw:?");
  ...
  else if(options & OPT_CTRL_REMOTE)
    {
      /* if using a remote control socket there should be no leftover args */
      if(arglist_len != 0 ||
	 split_addrport(opt_ctrl_remote, &ctrl_rem_name, &ctrl_rem_port) != 0)
	{
 ...
 * return one if there is a timeout value computed, zero otherwise.
 */
...
static int scamper(int argc, char *argv[])
{
  ...
  if(options & (OPT_CTRL_INET|OPT_CTRL_UNIX|OPT_CTRL_REMOTE))
	...
	{
	  if(scamper_control_init_remote(ctrl_rem_name,ctrl_rem_port) != 0)
	    return -1;
	}
  ...
  for(;;)
    {
      if(scamper_timeout(&timeout, &nextprobe, &lastprobe) != 0)
	{
	  /*
