 ...
 * sc_wartsdump
 *
 * $Id: sc_wartsdump.c,v 1.207 2015/08/28 05:14:00 mjl Exp $
 *
 *        Matthew Luckie
 *        mjl@luckie.org.nz
 *
...
static void dump_tbit(scamper_tbit_t *tbit)
{
  ...
  if(tbit->type == SCAMPER_TBIT_TYPE_PMTUD && tbit->data != NULL)
    ...
    {
      abc = tbit->data;
      printf(" icw: %u segments, L: %u", abc->icw, abc->limit);
      if(abc->last_ack != 0)
	printf(", last-ack: %u", abc->last_ack);
      if(tbit->result == SCAMPER_TBIT_RESULT_ABC_SUCCESS &&
	 scamper_tbit_abc_size(tbit, &u32) == 0)
	printf(", abc-size: %u bytes", u32);
      printf("\n");
      if(abc->ackc > 0)
	{
	  printf(" acks: ");
	  for(u8=0; u8<abc->ackc; u8++)
	    {
	      if(u8 > 0) printf(",");
	      printf("%u", abc->acks[u8]);
	    }
	  printf("\n");
	}
    }
